...
try{
	...
	/*
		either add a file called .app_keys.json in the same folder as app.js
		make it a json with the following content
		{
			"staging":{
				"stripe":{
	...
	*/
	...
	/*
	* if .app_keys.json is not present, you can add the keys here itself
	*/
	if(typeof(environments.staging.stripe.secret)=="string" && environments.staging.stripe.secret.length > 0 && typeof(environments.staging.mailgun.api_key)=="string" && environments.staging.mailgun.api_key.length > 0 && typeof(environments.production.stripe.secret)=="string" && environments.production.stripe.secret.length > 0 && typeof(environments.production.mailgun.api_key)=="string" && environments.production.mailgun.api_key.length > 0){
